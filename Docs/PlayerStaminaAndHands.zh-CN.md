...
- 空手站立、行走、奔跑和蹲下均启用双臂。正常低头即可看到手；平视时手位于视野下方。空手采用放松的手指姿态和左右交替摆臂，待机、走路、跑步按速度平滑混合。肩部后移并限制抬臂幅度，极低视角使用 60° 的姿态补偿上限，避免露出肩部模型断面。手电、持罐、饮用与收起动作仍使用现有资源。
...
| `Source/backrooms/Player/BRFirstPersonArmsComponent.cpp` | 空手启用双臂、补偿相机俯仰，按速度驱动 `EmptyLocomotion`；持物与一次性动作保留原有相机姿态 |
...
空手动画位于 `/Game/Gameplay/Player/Animations/`：`A_BR_Arms_RelaxedIdle/Walk/Run` 和 `BS_BR_Arms_EmptyLocomotion`。混合速度点为 0、350、600 cm/s，权重平滑速度为 6。`Tools/Animation/Build-RelaxedArms.py` 从项目现有全身动画生成匹配双臂骨架的资源，保留放松手型，缩小走跑幅度，并通过 `BRArmsAssetTools` 重建可供运行时使用的混合数据。需要重建时，在 Editor 编译完成后以 `-run=pythonscript -script=<脚本绝对路径> -EnablePlugins=PythonScriptPlugin,EditorScriptingUtilities` 执行。
...
- `-BRArmsViews -BRArmsMotionViews -UseFixedTimeStep -FPS=60 -BRInventoryCapture=<目录>`：按固定 60 Hz 模拟起步、行走、跑步和停步，记录左右摆臂并生成连续截图；使用 `-NullRHI` 且省略截图目录时额外检查逐帧手部位移，避免截图耗时干扰测量。
...
耐力初次交付的构建、测试日志及截图位于 `work/player_stamina_hands_20260908/`；运行证据不纳入 Git。该次 Editor 与 Game 的 Win64 Development 构建、13 项自动化、77 项背包回归、16 项实体运行检查、双客户端耐力测试及 10 张画面检查均已通过。

空手姿态修订的证据位于 `work/relaxed_hands_20260908/`：Editor/Game 构建、13 项自动化、77 项背包回归、10 个静态视角，以及固定 60 Hz 的 298 帧移动检查和 50 张连续截图均通过。未重新 Cook/Package。
