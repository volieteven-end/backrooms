...
对准可拾取的手电筒、电池组、杏仁水或任务钥匙时，物品上方显示一行 `按E拾取："物品名称"`，中心准星保持小点。提示跟随物品位置，移开视线、超出交互距离、受到遮挡、打开背包或拾取成功后隐藏。E 可直接拾取，原有左键交互仍可用。提示由 `BRGameHUDWidget` 定位，名称来自拾取物的 `GetInteractionText`；用 `-BRPickupPromptSmoke -BRItemCapture=<目录>` 可运行场景检查并保存四类物品截图。

当前空手双臂和耐力规则见 [玩家耐力与空手视角](PlayerStaminaAndHands.zh-CN.md)。下文保留 2026-09-06 的实现与验证记录，空手可见性和拾取提示以这两项更新为准。
